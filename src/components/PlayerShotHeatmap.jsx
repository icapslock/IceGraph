...
export const PlayerShotHeatmap = ({ playerId }) => {
  ...
  const getColor = (percentage) => {
    ...
    if (percentage < threshold) {
      return interpolateRgb("#ff0000", "#fff400")(percentage);
    } else {
      return interpolateRgb("#fff400", "#00ff00")(percentage);
    }
  };

  return (
    <div id="heatmap-container" style={{ position: 'relative' }}>
      <img src={`${process.env.PUBLIC_URL}/court.png`} alt="court" style={{ zIndex: 0 }} />
        ...
        <div style={{ position: 'relative', width: '520px', height: '619px' }}>
          {data.map(({ cx, cy, numShot, percentage }, i) => {
            ...
            return <div key={i} style={{
              position: 'absolute', bottom, left, width: diameter, height: diameter, backgroundColor: color, borderRadius: radius
            }} title={`x = ${cx}, y = ${cy}, # shots: ${numShot}, % goal: ${percentage * 100}%`}></div>;
          })}
        </div>
      </div>
    </div>
...
async function fetchData(playerId) {
  const response = await fetch(`https://statsapi.web.nhl.com/api/v1/people/${playerId}/stats?stats=gameLog&season=${SEASON}`);
  const data = await response.json();
  const gameIds = data['stats'][0]['splits'].map(split => split['game']['gamePk']);
  ...
  await Promise.all(gameIds.map(async gameId => {
    ...
    data['liveData']['plays']['allPlays']
      .filter(playEvent => ['Shot', 'Goal'].includes(playEvent['result']['event']))
      .forEach(playEvent => {
